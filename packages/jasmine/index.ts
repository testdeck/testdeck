...
class JasmineAdapter implements core.TestFrameworkAdapter {
    ...
    switch (options.execution) {
      ...
      case "focus":
        core.withCondition(options.condition, () => {
          fdescribe(options.name, callback);
        }, () => {
          describe(options.name, callback);
        });
        break;
      ...
      case "pending":
        core.withCondition(options.condition, () => {
          xdescribe(options.name, callback);
        }, () => {
          describe(options.name, callback);
        });
        break;
  ...
  it(options: core.EffectiveTestOptions, callback: () => Promise<unknown> | void): void {
    switch (options.execution) {
      ...
      case "focus":
        core.withCondition(options.condition, () => {
          fit(options.name, callback, options.timeout);
        }, () => {
          it(options.name, callback, options.timeout);
        });
        break;
      ...
      case "pending":
        core.withCondition(options.condition, () => {
          xit(options.name, callback, options.timeout);
        }, () => {
          it(options.name, callback, options.timeout);
        });
        break;
...
const testUI = new JasmineClassTestUI(new JasmineAdapter());
