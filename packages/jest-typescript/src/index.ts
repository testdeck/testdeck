...
function applyDecorators(mocha: any, ctorOrProto, method, instance) {
...
function applyTestTraits(context: any, instance: SuiteProto, method: Function) {
...
function applySuiteTraits(context: any, target: SuiteCtor) {
...
function suiteClassCallback(target: SuiteCtor, context: TestFunctions) {
  return function() {
    ...
    let beforeEachFunction: ProvidesOptionalCallback;
    let beforeTimeout;
    if (prototype.before) {
      beforeTimeout = prototype.before[timeoutSymbol];
      if (isAsync(prototype.before)) {
        beforeEachFunction = wrapNameAndToString(function(this: any, done: Function) {
        ...
        beforeEachFunction = wrapNameAndToString(function(this: any) {
          instance = getInstance(target);
          applyDecorators(this, prototype, prototype.before, instance);
      ...
      beforeEachFunction = function(this: any) {
        instance = getInstance(target);
      };
    }
    context.beforeEach(beforeEachFunction, beforeTimeout);

    let afterEachFunction: ProvidesOptionalCallback;
    let afterTimeout;
    if (prototype.after) {
      afterTimeout = prototype.after[timeoutSymbol];
      if (isAsync(prototype.after)) {
        afterEachFunction = wrapNameAndToString(function(this: any, done) {
          try {
        ...
        afterEachFunction = wrapNameAndToString(function(this: any) {
          try {
            applyDecorators(this, prototype, prototype.after, instance);
      ...
      afterEachFunction = function(this: any) {
        instance = undefined;
      };
    }
    context.afterEach(afterEachFunction, afterTimeout);
    ...
    function runTest(prototype: any, method: Function) {
      ...
      const timeout = method[timeoutSymbol];
      ...
      if (parameters) {
        ...
        context.describe(testName, function() {
          ...
          parameters.forEach((parameterOptions, i) => {
            ...
            applyTestFunc(testFunc, timeout, parametersTestName, method, [params]);
          });
        });
      } else {
        const testFunc = (shouldSkip && context.it.skip)
                         || (shouldOnly && context.it.only)
                         || (shouldPending && context.it.todo)
                         || context.it;

        applyTestFunc(testFunc, timeout, testName, method, []);
      }
    }
    ...
    function applyTestFunc(testFunc: Function, timeout: any, testName: string, method: Function, callArgs: any[]) {
      if (isAsync(method)) {
        testFunc(testName, wrapNameAndToString(function(this: any, done) {
        ...
        const t = testFunc(testName, wrapNameAndToString(function(this: any) {
          applyDecorators(this, prototype, method, instance);
          applyTestTraits(this, instance, method);
    ...
    while (currentPrototype !== objPrototype) {
      ...
      if ((currentPrototype !== objPrototype) && currentPrototype.constructor[suiteSymbol]) {
...
function makeTestFunction(mark?: string | symbol) {
  return testOverload({
    testProperty(target: Object, propertyKey: string | symbol, descriptor?: PropertyDescriptor): void {
      target[propertyKey][testNameSymbol] = propertyKey.toString();
      if (mark) {
...
function makeTestObject(context: TestFunctions): TestDecorator {
  return Object.assign(makeTestFunction(), {
...
function createNumericBuiltinTrait(traitSymbol: any, fn: (ctx: any, value: number) => void): NumericDecoratorOrTrait {
...
export const slow = createNumericBuiltinTrait(slowSymbol, (context, value) => { return; });
...
export const timeout = createNumericBuiltinTrait(timeoutSymbol, (context, value) => { return; });

/**
 * Set a test method or site retries count.
 * @param value The number of retries to attempt when running the test.
 */
// export const retries = createNumericBuiltinTrait(retriesSymbol, (context: any, value) => context.retries(value));
export const retries = createNumericBuiltinTrait(retriesSymbol, (context: any, value) => { return; });

export const skipOnError: SuiteTrait = trait(function() {
  throw new Error("skipOnError is not supported under jest/jasmine");
});
...
export const context: PropertyDecorator = function context(target: Object, propertyKey: string): void {
  throw new Error("@context is not available under jest/jasmine");
